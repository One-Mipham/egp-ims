...
from app.schemas import (
    ...
    VehicleSupplierCreate,
    VehicleSupplierUpdate,
    VehicleSupplierResponse,
    VehiclePurchaseCreate,
    VehiclePurchaseUpdate,
    ...
    StockGiftUpdate,
    StockGiftResponse,
    StockGiftPurchaseCreate,
    StockGiftPurchaseUpdate,
    StockGiftPurchaseResponse,
    StockGiftRequisitionCreate,
    StockGiftRequisitionUpdate,
    StockGiftRequisitionResponse,
    StockGiftInboundCreate,
    StockGiftInboundUpdate,
    StockGiftInboundResponse,
    StockGiftOutboundCreate,
    StockGiftOutboundUpdate,
    StockGiftOutboundResponse,
)
...
def _process_approval(db: Session, target_type: str, target_id: int, user: User, action: str, comment: str | None):
    ...
    if action == "approve":
        ...
        _write_audit(
            db, user.company_id if hasattr(user, "company_id") else 0, user, "approve_step", target_type, target_id
        )
    elif action == "reject":
        ...
        _write_audit(
            db, user.company_id if hasattr(user, "company_id") else 0, user, "reject_step", target_type, target_id
        )
...
def _submit_for_approval(
    db: Session, company_id: int, target_type: str, target_id: int, approver_ids: list[int], user: User
):
...
def list_vehicle_suppliers(
    company_id: int = Query(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    return (
        db.query(VehicleSupplier).filter(VehicleSupplier.company_id == company_id).order_by(VehicleSupplier.name).all()
    )


@router.post("/vehicles/suppliers", response_model=VehicleSupplierResponse)
def create_vehicle_supplier(
    data: VehicleSupplierCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_vehicle_purchases(
    company_id: int = Query(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    return (
        db.query(VehiclePurchase)
        .filter(VehiclePurchase.company_id == company_id)
...
def create_vehicle_purchase(
    data: VehiclePurchaseCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_vehicle_maintenance(
    ...
    return (
        ...
        .filter(VehicleMaintenance.company_id == company_id)
...
def create_vehicle_maintenance(
    data: VehicleMaintenanceCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def create_insurance(
    data: InsurancePolicyCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_stock_categories(
    ...
    return (
        ...
        .filter(StockCategory.company_id == company_id, StockCategory.is_active)
        .order_by(StockCategory.code)
        .all()
    )
...
def create_stock_category(
    data: StockCategoryCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_stock_purchases(
    ...
    return (
        ...
        .filter(StockPurchase.company_id == company_id)
...
def create_stock_purchase(
    data: StockPurchaseCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_stock_requisitions(
    company_id: int = Query(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    return (
        db.query(StockRequisition)
        .filter(StockRequisition.company_id == company_id)
        .order_by(StockRequisition.created_at.desc())
        .all()
    )
...
def create_stock_requisition(
    data: StockRequisitionCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_stock_inbound(
    company_id: int = Query(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    return (
        db.query(StockInbound)
        .filter(StockInbound.company_id == company_id)
        .order_by(StockInbound.created_at.desc())
        .all()
    )
...
def create_stock_inbound(
    data: StockInboundCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_stock_outbound(
    company_id: int = Query(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    return (
        db.query(StockOutbound)
        .filter(StockOutbound.company_id == company_id)
        .order_by(StockOutbound.created_at.desc())
        .all()
    )


@router.post("/stock/assets/outbound", response_model=StockOutboundResponse)
def create_stock_outbound(
    data: StockOutboundCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_gift_categories(
    ...
    return (
        ...
        .filter(GiftCategory.company_id == company_id, GiftCategory.is_active)
...
def create_gift_category(
    data: GiftCategoryCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_gift_purchases(
    company_id: int = Query(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    return (
        db.query(StockGiftPurchase)
        .filter(StockGiftPurchase.company_id == company_id)
        .order_by(StockGiftPurchase.created_at.desc())
        .all()
    )
...
def create_gift_purchase(
    data: StockGiftPurchaseCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_gift_requisitions(
    ...
    return (
        db.query(StockGiftRequisition)
        .filter(StockGiftRequisition.company_id == company_id)
        .order_by(StockGiftRequisition.created_at.desc())
        .all()
    )
...
def create_gift_requisition(
    data: StockGiftRequisitionCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_gift_inbound(
    company_id: int = Query(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    return (
        db.query(StockGiftInbound)
        .filter(StockGiftInbound.company_id == company_id)
        .order_by(StockGiftInbound.created_at.desc())
        .all()
    )


@router.post("/stock/gifts/inbound", response_model=StockGiftInboundResponse)
def create_gift_inbound(
    data: StockGiftInboundCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
...
def list_gift_outbound(
    company_id: int = Query(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
    return (
        db.query(StockGiftOutbound)
        .filter(StockGiftOutbound.company_id == company_id)
        .order_by(StockGiftOutbound.created_at.desc())
        .all()
    )


@router.post("/stock/gifts/outbound", response_model=StockGiftOutboundResponse)
def create_gift_outbound(
    data: StockGiftOutboundCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)
):
