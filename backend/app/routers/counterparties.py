...
def create_counterparty(company_id: int, data: CounterpartyUpdate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    import random
    ...
    for field in ['short_name', 'category', 'tax_number', 'bank_account', 'bank_name', 'address', 'phone', 'contact_person', 'website', 'email', 'zip_code']:
...
def update_counterparty(counterparty_id: int, data: CounterpartyUpdate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    cp = db.query(Counterparty).filter(Counterparty.id == counterparty_id).first()
    if not cp:
        raise HTTPException(status_code=404, detail="客户不存在")
    for field in ['name', 'short_name', 'category', 'tax_number', 'bank_account', 'bank_name', 'address', 'phone', 'contact_person', 'website', 'email', 'zip_code']:
        val = getattr(data, field, None)
        if val is not None:
            setattr(cp, field, val)
    db.commit()
...
def import_counterparties(company_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    for row in ws.iter_rows(min_row=3, max_row=ws.max_row, values_only=True):
        ...
        if not code or not name:
            continue
        existing = db.query(Counterparty).filter(Counterparty.company_id == company_id, Counterparty.code == code).first()
        if not existing:
            db.add(Counterparty(
