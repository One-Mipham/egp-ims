...
def create_department(data: DepartmentCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    dept = Department(company_id=data.company_id, name=data.name, code=data.code, manager=data.manager)
    db.add(dept)
    db.commit()
    db.refresh(dept)
    return dept
...
def bulk_import_departments(company_id: int, rows: list[dict], db: Session = Depends(get_db), user=Depends(get_current_user)):
    """批量导入部门。每行: {code, name, manager?, is_active?}"""
    imported = 0
    errors = []
    for i, row in enumerate(rows):
        ...
        if existing:
            ...
            dept = Department(company_id=company_id, code=code, name=name, manager=manager, is_active=is_active)
            db.add(dept)
        imported += 1
    db.commit()
