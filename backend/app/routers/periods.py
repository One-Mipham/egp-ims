...
from app.schemas import (
    ClosePeriodRequest,
    PeriodResponse,
    CarryForwardEntryCreate,
    CarryForwardEntryResponse,
    CloseCheckResult,
...
def list_periods(
    ...
    return (
        ...
        .filter(AccountingPeriod.company_id == company_id)
...
def close_period(
    ...
    if carry_result.get("skipped"):
        carry_detail = {
            "carry_forward": "skipped",
            "reason": carry_result.get("reason"),
        }
    else:
        carry_detail = {
...
def get_quarterly_summary(
    ...
    closed_periods = {
        p.period
        for p in db.query(AccountingPeriod)
        .filter(
            AccountingPeriod.company_id == company_id,
...
def execute_carry_forward(
    ...
    now = datetime.now(timezone.utc)
    voucher_no = f"转字{period.replace('-', '')}-{db.query(Voucher).filter(Voucher.company_id == company_id, Voucher.date.startswith(period)).count() + 1:04d}"  # noqa: E501
