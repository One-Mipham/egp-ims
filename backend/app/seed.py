...
def seed_kb_categories(db: Session, company_id: int = 1):
    ...
    for sort, (l1_name, l2_names) in enumerate(KB_SEED_CATEGORIES.items()):
        l1 = KbCategory(
            company_id=company_id,
            name=l1_name,
            parent_id=None,
    ...
    db.commit()
    print(
        f"  ✅ KB 分类已创建: {len(KB_SEED_CATEGORIES)} 个一级, {sum(len(v) for v in KB_SEED_CATEGORIES.values())} 个二级"  # noqa: E501
    )
...
SUBSCRIPTION_PLANS = [
    {
        ...
        "price_cny": 299,
        "price_usd": 49,
        "modules": ["accounting", "reports", "receivables", "payables", "expenses"],
        "sort_order": 1,
    },
    {
        "name": "基础包-半年",
...
def seed_cashflow_items(db: Session, company_id: int):
    ...
    CF_ITEMS = [
        ...
        ("CF03", "支付的各项税费", "op_tax", "outflow", "2221", "1001,1002"),
        (
            "CF04",
            "支付其它与经营活动有关的现金",
            "op_other_out",
            "outflow",
            "6602,6603,2241,1221,6601",
            "1001,1002",
        ),
        # === 投资活动流出 ===
        (
