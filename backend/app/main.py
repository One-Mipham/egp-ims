...
async def company_isolation_middleware(request: Request, call_next):
    ...
    if request.method in ("POST", "PUT", "PATCH", "DELETE") and (
        qp_cid or (request.method in ("POST", "PUT", "PATCH") and body)
    ):
        cid = None
        if qp_cid:
        ...
        if cid:
            ...
            try:
                ...
                if _company and _company.subscription_status in (
                    "expired",
                    "past_due",
                    "cancelled",
                ):
...
app.include_router(
    inventory_trade.router,
    prefix="/api/inventory-trade",
    tags=["进销存管理"],
    dependencies=_inv,
)
