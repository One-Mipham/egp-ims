...
def verify_balances(db: Session, company_id: int):
    ...
    print("  科目发生额汇总:")
    rows = (
        db.query(VE.account_code, sa_func.sum(VE.debit), sa_func.sum(VE.credit))
        .join(Voucher)
        .filter(Voucher.company_id == company_id)
        .group_by(VE.account_code)
        .order_by(sa_func.sum(VE.debit) + sa_func.sum(VE.credit))
        .all()
    )
    ...
    for month in ["2026-01", "2026-02", "2026-03", "2026-04"]:
        ...
        revenue = (
            db.query(sa_func.sum(VE.credit))
            .join(Voucher)
            .join(Account, VE.account_code == Account.code)
            .filter(
        ...
        fin_exp = (
            db.query(sa_func.sum(VE.debit) - sa_func.sum(VE.credit))
            .join(Voucher)
            .join(Account, VE.account_code == Account.code)
            .filter(
                Voucher.company_id == company_id,
        ...
        print(
            f"    {month}: 收入={revenue:>12,.2f}  管理费={mgmt_exp:>12,.2f}  财务费={fin_exp:>10,.2f}  利润={profit:>12,.2f}"  # noqa: E501
        )
