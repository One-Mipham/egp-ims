...
def run_migrations():
    ...
    migrations = [
        ...
        "CREATE TABLE IF NOT EXISTS cashflow_plans (id INTEGER PRIMARY KEY AUTOINCREMENT, company_id INTEGER NOT NULL REFERENCES companies(id), name VARCHAR(100) NOT NULL, year INTEGER NOT NULL, status VARCHAR(10) DEFAULT 'draft', created_by INTEGER REFERENCES users(id), created_at TIMESTAMP)",
        "CREATE TABLE IF NOT EXISTS cashflow_plan_items (id INTEGER PRIMARY KEY AUTOINCREMENT, plan_id INTEGER NOT NULL REFERENCES cashflow_plans(id), account_code VARCHAR(10) NOT NULL, month VARCHAR(7) NOT NULL, amount FLOAT DEFAULT 0)",
