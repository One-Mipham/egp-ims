...
for a in accounts:
    ...
    end = init + md - mc if a.balance_direction == "debit" else init + mc - md
...
for m in range(1, 6):
    ...
    for v in mvs:
        for e in db.query(VoucherEntry).filter(VoucherEntry.voucher_id == v.id).all():
            ...
            entries += 1
    print(
        f"  {month}: {len(mvs):3d} vouchers, {entries:4d} entries, Dr={md:>14,.2f}, Cr={mc:>14,.2f}, {'OK' if abs(md - mc) < 0.01 else 'IMBAL'}"  # noqa: E501
    )
