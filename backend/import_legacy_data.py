...
def create_period_vouchers(db, company_id):
    ...
    for voucher_date, month_label in months:
        ...
        total_created += 1
        print(
            f"  ✓ {month_label}: voucher_no={voucher.voucher_no}, entries={len(entries)}, dr={total_dr}, cr={total_cr}"
        )
...
def main():
    ...
    print(
        f"  科目: {acct_count} | 部门: {dept_count} | 往来单位: {cp_count} | 员工: {person_count} | 凭证: {voucher_count}"  # noqa: E501
    )
