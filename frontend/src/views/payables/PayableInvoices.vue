...
<template>
  <div class="p-4">
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-lg font-bold">{{ t('payables.invoices') }}</h1>
    ...
    <table class="w-full text-sm border-collapse">
      ...
      <tbody>
        <tr v-for="item in items" :key="item.id" class="hover:bg-zinc-50">
          ...
          <td class="p-2 border">
            <span :class="item.status === '已付款' ? 'text-green-600' : 'text-amber-600'">{{ item.status }}</span>
          </td>
          <td class="p-2 border">
