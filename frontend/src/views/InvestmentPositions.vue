    ...
    <Dialog v-model:visible="showDialog" :header="editingId ? '编辑持仓' : '新增持仓'" :modal="true" class="w-[500px]">
      <div class="flex flex-col gap-3">
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-sm mb-1">所属组合</label><Dropdown v-model="form.portfolio_id" :options="portfolios" optionLabel="name" optionValue="id" class="w-full" /></div>
          <div><label class="block text-sm mb-1">会计科目 *</label><Dropdown v-model="form.account_code" :options="ACCOUNT_OPTIONS" optionLabel="label" optionValue="value" class="w-full" filter /></div>
        </div>
        <div class="grid grid-cols-2 gap-3">
