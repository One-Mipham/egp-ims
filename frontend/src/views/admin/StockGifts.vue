...
async function load() {
  ...
  try {
    const [r1, r2] = await Promise.all([
      listStockGifts(companyId.value, categoryFilter.value || undefined),
      listGiftCategories(companyId.value),
    ])
    items.value = r1.data
...
async function save() {
  try {
    if (isEdit.value && editId.value) await updateStockGift(editId.value, form.value)
    else await createStockGift(form.value)
  ...
  <div>
    ...
    <div class="bg-white rounded-sm border border-stone-200 overflow-x-auto">
      <DataTable :value="items" :loading="loading" stripedRows size="small" paginator :rows="15">
        ...
        <Column field="current_stock" header="当前库存" sortable>
          <template #body="{ data }"
            ><Tag
              :value="data.current_stock.toString()"
              :severity="data.current_stock <= 5 ? 'warn' : data.current_stock === 0 ? 'danger' : 'success'"
          /></template>
        </Column>
    ...
    >
      <div class="grid grid-cols-2 gap-3">
        ...
        <div>
          <label class="block text-xs text-zinc-500 mb-1">单位</label
          ><Dropdown v-model="form.unit" :options="['个', '箱', '盒', '套', '件']" class="w-full" />
        </div>
        <div>
          <label class="block text-xs text-zinc-500 mb-1">当前库存</label
          ><InputNumber v-model="form.current_stock" class="w-full" />
        </div>
        <div>
