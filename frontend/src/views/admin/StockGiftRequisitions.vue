...
import { listGiftRequisitions, createGiftRequisition, updateGiftRequisition, deleteGiftRequisition, submitGiftRequisition } from '@/api'
...
const emptyForm = () => ({ company_id: companyId.value, gift_id: null, applicant: '', department: '', quantity: 0, recipient: '', recipient_organization: '', reason: '' })
const form = ref(emptyForm())

async function load() { loading.value = true; try { const { data } = await listGiftRequisitions(companyId.value); items.value = data } finally { loading.value = false } }
...
async function save() { try { if (isEdit.value && editId.value) await updateGiftRequisition(editId.value, form.value); else await createGiftRequisition(form.value); showDialog.value = false; await load() } catch (e: any) { alert(e.response?.data?.detail || '操作失败') } }
async function remove(id: number) { if (!confirm('确定删除？')) return; try { await deleteGiftRequisition(id); await load() } catch (e: any) { alert('删除失败') } }
async function doSubmit(id: number) { const ids = prompt('请输入审批人ID（逗号分隔）：'); if (!ids) return; try { await submitGiftRequisition(id, ids.split(',').map(Number)); await load() } catch (e: any) { alert('提交失败') } }
...
<template>
  <div>
    ...
    <Dialog v-model:visible="showDialog" :header="isEdit ? '编辑领用申请' : '新建领用申请'" :style="{ width: '500px' }" :modal="true">
      <div class="grid grid-cols-2 gap-3">
        <div class="col-span-2"><label class="block text-xs text-zinc-500 mb-1">礼品ID</label><InputNumber v-model="form.gift_id" class="w-full" /></div>
        ...
        <div class="col-span-2"><label class="block text-xs text-zinc-500 mb-1">理由</label><Textarea v-model="form.reason" rows="2" class="w-full" /></div>
      </div>
