...
import { listGiftInbound, createGiftInbound, updateGiftInbound, deleteGiftInbound, submitGiftInbound, listStockGifts } from '@/api'
...
const emptyForm = () => ({ company_id: companyId.value, gift_id: 0, inbound_type: '采购', quantity: 0, unit_price: 0, supplier: '', inbound_date: new Date().toISOString().slice(0,10), receiver: '', notes: '' })
const form = ref(emptyForm())

async function load() { loading.value = true; try { const [r1, r2] = await Promise.all([listGiftInbound(companyId.value), listStockGifts(companyId.value)]); items.value = r1.data; gifts.value = r2.data } finally { loading.value = false } }
...
async function save() { try { if (isEdit.value && editId.value) await updateGiftInbound(editId.value, form.value); else await createGiftInbound(form.value); showDialog.value = false; await load() } catch (e: any) { alert(e.response?.data?.detail || '操作失败') } }
async function remove(id: number) { if (!confirm('确定删除？')) return; try { await deleteGiftInbound(id); await load() } catch (e: any) { alert('删除失败') } }
async function doSubmit(id: number) { const ids = prompt('请输入审批人ID（逗号分隔）：'); if (!ids) return; try { await submitGiftInbound(id, ids.split(',').map(Number)); await load() } catch (e: any) { alert('提交失败') } }
...
<template>
  <div>
    <div class="flex justify-end mb-4"><Button label="新建礼品入库" icon="pi pi-plus" @click="openCreate" /></div>
    ...
    <Dialog v-model:visible="showDialog" :header="isEdit ? '编辑入库' : '新建入库'" :style="{ width: '450px' }" :modal="true">
      <div class="grid gap-3">
        <div><label class="block text-xs text-zinc-500 mb-1">礼品</label><Dropdown v-model="form.gift_id" :options="gifts" optionLabel="name" optionValue="id" placeholder="选择礼品" class="w-full" filter /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">入库类型</label><Dropdown v-model="form.inbound_type" :options="['采购', '接收']" class="w-full" /></div>
        ...
        <div><label class="block text-xs text-zinc-500 mb-1">入库日期</label><InputText v-model="form.inbound_date" type="date" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">接收人</label><InputText v-model="form.receiver" class="w-full" /></div>
