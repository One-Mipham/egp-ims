...
const emptyForm = () => ({ company_id: companyId.value, name: '', contact_person: '', contact_phone: '', brands_carried: '', notes: '' })
const form = ref(emptyForm())

async function load() { loading.value = true; try { const { data } = await listVehicleSuppliers(companyId.value); items.value = data } finally { loading.value = false } }
...
async function save() {
  try { if (isEdit.value && editId.value) await updateVehicleSupplier(editId.value, form.value); else await createVehicleSupplier(form.value); showDialog.value = false; await load() }
  catch (e: any) { alert(e.response?.data?.detail || '操作失败') }
}
async function remove(id: number) { if (!confirm('确定删除？')) return; try { await deleteVehicleSupplier(id); await load() } catch (e: any) { alert('删除失败') } }
onMounted(load)
</script>
...
<template>
  <div>
    ...
    <Dialog v-model:visible="showDialog" :header="isEdit ? '编辑供应商' : '新增供应商'" :style="{ width: '450px' }" :modal="true">
      <div class="grid gap-3">
        <div><label class="block text-xs text-zinc-500 mb-1">名称</label><InputText v-model="form.name" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">联系人</label><InputText v-model="form.contact_person" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">电话</label><InputText v-model="form.contact_phone" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">代理品牌</label><InputText v-model="form.brands_carried" class="w-full" /></div>
