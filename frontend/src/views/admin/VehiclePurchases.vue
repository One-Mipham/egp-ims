...
async function load() {
  loading.value = true
  try {
    const { data } = await listVehiclePurchases(companyId.value)
    items.value = data
  } finally {
    loading.value = false
  }
  ...
  <div>
    ...
    >
      <div class="grid grid-cols-2 gap-3">
        ...
        <div>
          <label class="block text-xs text-zinc-500 mb-1">品牌</label
          ><InputText v-model="form.vehicle_brand" class="w-full" />
        </div>
        <div>
          <label class="block text-xs text-zinc-500 mb-1">型号</label
          ><InputText v-model="form.vehicle_model" class="w-full" />
        </div>
        <div>
          <label class="block text-xs text-zinc-500 mb-1">预估价格</label
          ><InputNumber v-model="form.estimated_price" class="w-full" />
        </div>
        <div>
          <label class="block text-xs text-zinc-500 mb-1">供应商名称</label
          ><InputText v-model="form.supplier_name" class="w-full" />
        </div>
        <div class="col-span-2">
          <label class="block text-xs text-zinc-500 mb-1">配置要求</label
          ><Textarea v-model="form.configuration" rows="2" class="w-full" />
        </div>
        <div class="col-span-2">
