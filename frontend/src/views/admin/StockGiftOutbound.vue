...
async function load() {
  loading.value = true
  try {
    const [r1, r2] = await Promise.all([listGiftOutbound(companyId.value), listStockGifts(companyId.value)])
    items.value = r1.data
    gifts.value = r2.data
  } finally {
    loading.value = false
...
async function save() {
  try {
    if (isEdit.value && editId.value) await updateGiftOutbound(editId.value, form.value)
    else await createGiftOutbound(form.value)
    ...
    >
      <div class="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          ><Dropdown v-model="form.outbound_type" :options="['赠送', '发放']" class="w-full" />
        </div>
        <div>
