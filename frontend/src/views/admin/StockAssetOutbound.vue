...
import { listStockOutbound, createStockOutbound, updateStockOutbound, deleteStockOutbound, submitStockOutbound, listStockAssets } from '@/api'
...
const emptyForm = () => ({ company_id: companyId.value, asset_id: 0, outbound_type: '领用', quantity: 1, recipient: '', outbound_date: new Date().toISOString().slice(0,10), notes: '' })
const form = ref(emptyForm())

async function load() { loading.value = true; try { const [r1, r2] = await Promise.all([listStockOutbound(companyId.value), listStockAssets(companyId.value)]); items.value = r1.data; assets.value = r2.data } finally { loading.value = false } }
...
async function save() { try { if (isEdit.value && editId.value) await updateStockOutbound(editId.value, form.value); else await createStockOutbound(form.value); showDialog.value = false; await load() } catch (e: any) { alert(e.response?.data?.detail || '操作失败') } }
async function remove(id: number) { if (!confirm('确定删除？')) return; try { await deleteStockOutbound(id); await load() } catch (e: any) { alert('删除失败') } }
async function doSubmit(id: number) { const ids = prompt('请输入审批人ID（逗号分隔）：'); if (!ids) return; try { await submitStockOutbound(id, ids.split(',').map(Number)); await load() } catch (e: any) { alert('提交失败') } }
...
<template>
  <div>
    ...
    <Dialog v-model:visible="showDialog" :header="isEdit ? '编辑出库' : '新建出库'" :style="{ width: '450px' }" :modal="true">
      <div class="grid gap-3">
        <div><label class="block text-xs text-zinc-500 mb-1">资产</label><Dropdown v-model="form.asset_id" :options="assets" optionLabel="name" optionValue="id" placeholder="选择资产" class="w-full" filter /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">出库类型</label><Dropdown v-model="form.outbound_type" :options="['领用', '调拨', '报废出库']" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">数量</label><InputNumber v-model="form.quantity" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">领用人</label><InputText v-model="form.recipient" class="w-full" /></div>
