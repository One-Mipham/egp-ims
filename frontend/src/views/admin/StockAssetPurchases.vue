...
import { listStockPurchases, createStockPurchase, updateStockPurchase, deleteStockPurchase, submitStockPurchase } from '@/api'
...
const emptyForm = () => ({ company_id: companyId.value, applicant: '', department: '', asset_name: '', category: '', quantity: 1, estimated_price: 0, reason: '' })
const form = ref(emptyForm())

async function load() { loading.value = true; try { const { data } = await listStockPurchases(companyId.value); items.value = data } finally { loading.value = false } }
...
async function save() {
  try { if (isEdit.value && editId.value) await updateStockPurchase(editId.value, form.value); else await createStockPurchase(form.value); showDialog.value = false; await load() }
  catch (e: any) { alert(e.response?.data?.detail || '操作失败') }
}
async function remove(id: number) { if (!confirm('确定删除？')) return; try { await deleteStockPurchase(id); await load() } catch (e: any) { alert('删除失败') } }
async function doSubmit(id: number) { const ids = prompt('请输入审批人ID（逗号分隔）：'); if (!ids) return; try { await submitStockPurchase(id, ids.split(',').map(Number)); await load() } catch (e: any) { alert('提交失败') } }
  ...
  <div>
    ...
    <Dialog v-model:visible="showDialog" :header="isEdit ? '编辑采购申请' : '新建采购申请'" :style="{ width: '500px' }" :modal="true">
      <div class="grid grid-cols-2 gap-3">
        <div class="col-span-2"><label class="block text-xs text-zinc-500 mb-1">资产名称</label><InputText v-model="form.asset_name" class="w-full" /></div>
        ...
        <div><label class="block text-xs text-zinc-500 mb-1">类别</label><InputText v-model="form.category" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">数量</label><InputNumber v-model="form.quantity" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">预估单价</label><InputNumber v-model="form.estimated_price" class="w-full" /></div>
