...
async function load() {
  loading.value = true
  try {
    const [r1, r2] = await Promise.all([listStockCounts(companyId.value), listStockAssets(companyId.value)])
    items.value = r1.data
...
async function save() {
  ...
  try {
    if (isEdit.value && editId.value) await updateStockCount(editId.value, form.value)
    else await createStockCount(form.value)
  ...
  <div>
    ...
    <div class="bg-white rounded-sm border border-stone-200 overflow-x-auto">
      <DataTable :value="items" :loading="loading" stripedRows size="small" paginator :rows="15">
        ...
        <Column header="差异" style="min-width: 80px"
          ><template #body="{ data }"
            ><span :class="data.discrepancy !== 0 ? 'text-red-600 font-semibold' : ''">{{
              data.discrepancy
            }}</span></template
          ></Column
        >
    ...
    <Dialog
      ...
      :header="isEdit ? '编辑盘库' : '新建盘库'"
      :style="{ width: '450px' }"
      :modal="true"
    >
      <div class="grid gap-3">
        <div>
          <label class="block text-xs text-zinc-500 mb-1">盘库日期</label
          ><InputText v-model="form.count_date" type="date" class="w-full" />
        </div>
        <div>
        ...
        <div>
          <label class="block text-xs text-zinc-500 mb-1">账面数量</label
          ><InputNumber v-model="form.book_quantity" class="w-full" />
        </div>
        <div>
          <label class="block text-xs text-zinc-500 mb-1">实盘数量</label
          ><InputNumber v-model="form.actual_quantity" class="w-full" />
        </div>
        <div>
          <label class="block text-xs text-zinc-500 mb-1">差异</label
          ><InputText
            :model-value="String(discrepancyAuto)"
            readonly
            class="w-full"
            :class="discrepancyAuto !== 0 ? 'text-red-600' : ''"
          />
        </div>
        <div>
          <label class="block text-xs text-zinc-500 mb-1">盘点人</label
          ><InputText v-model="form.counter" class="w-full" />
        </div>
        <div>
          <label class="block text-xs text-zinc-500 mb-1">差异原因</label
