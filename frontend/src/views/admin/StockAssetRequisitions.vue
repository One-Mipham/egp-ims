...
import { listStockRequisitions, createStockRequisition, updateStockRequisition, deleteStockRequisition, submitStockRequisition } from '@/api'
...
const emptyForm = () => ({ company_id: companyId.value, asset_id: null, applicant: '', department: '', quantity: 1, reason: '' })
const form = ref(emptyForm())

async function load() { loading.value = true; try { const { data } = await listStockRequisitions(companyId.value); items.value = data } finally { loading.value = false } }
...
async function save() {
  try { if (isEdit.value && editId.value) await updateStockRequisition(editId.value, form.value); else await createStockRequisition(form.value); showDialog.value = false; await load() }
  catch (e: any) { alert(e.response?.data?.detail || '操作失败') }
}
async function remove(id: number) { if (!confirm('确定删除？')) return; try { await deleteStockRequisition(id); await load() } catch (e: any) { alert('删除失败') } }
async function doSubmit(id: number) { const ids = prompt('请输入审批人ID（逗号分隔）：'); if (!ids) return; try { await submitStockRequisition(id, ids.split(',').map(Number)); await load() } catch (e: any) { alert('提交失败') } }
...
<template>
  <div>
    ...
    <Dialog v-model:visible="showDialog" :header="isEdit ? '编辑领用申请' : '新建领用申请'" :style="{ width: '450px' }" :modal="true">
      <div class="grid gap-3">
        <div><label class="block text-xs text-zinc-500 mb-1">资产ID</label><InputNumber v-model="form.asset_id" class="w-full" /></div>
        ...
        <div><label class="block text-xs text-zinc-500 mb-1">领用理由</label><Textarea v-model="form.reason" rows="2" class="w-full" /></div>
      </div>
