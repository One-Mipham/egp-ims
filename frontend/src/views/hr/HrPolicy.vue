...
import { listHrPositions, createHrPosition, updateHrPosition, deleteHrPosition, listHrPolicies, upsertHrPolicy } from '@/api'
...
const levelOptions = [
  { label: '1 - 董事会', value: 1 }, { label: '2 - 监事会', value: 2 },
  { label: '3 - 管理层', value: 3 }, { label: '4 - 部门正职', value: 4 },
  { label: '5 - 部门副职', value: 5 }, { label: '6 - 中层', value: 6 },
  { label: '7 - 基层', value: 7 },
]
...
async function load() {
  ...
  try {
    const [pr, po] = await Promise.all([
      listHrPolicies(companyId.value),
...
<template>
  <div class="space-y-6">
    ...
    <div class="form-card">
      ...
      <Textarea v-model="policyContent" rows="12" class="w-full text-sm font-mono" placeholder="在此编写公司人力资源管理制度..." />
    ...
    <div class="form-card">
      ...
      <DataTable :value="positions" :loading="loading" stripedRows class="text-xs">
        <Column field="level" header="层级" style="width:100px">
          <template #body="{ data }">{{ levelOptions.find(l => l.value === data.level)?.label || data.level }}</template>
        </Column>
        ...
        <Column header="操作" style="width:120px">
          <template #body="{ data }">
            ...
            <Button v-if="data.is_active" label="停用" text severity="danger" size="small" @click="handleDeletePos(data.id)" />
          </template>
        </Column>
      </DataTable>
    </div>
    ...
    <Dialog v-model:visible="showPosDialog" :header="editingPos ? '编辑职级' : '新增职级'" :style="{ width: '400px' }">
      <div class="flex flex-col gap-3 py-4">
        <div><label class="block text-xs text-zinc-500 mb-1">职位名称</label><InputText v-model="posForm.name" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">层级</label><Dropdown v-model="posForm.level" :options="levelOptions" option-label="label" option-value="value" class="w-full" /></div>
        <div><label class="block text-xs text-zinc-500 mb-1">排序号</label><InputNumber v-model="posForm.sort_order" class="w-full" /></div>
