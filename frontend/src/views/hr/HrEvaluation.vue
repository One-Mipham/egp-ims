...
const evaluations = ref<any[]>([]); const employees = ref<any[]>([])
...
const form = ref({ employee_id: null as number | null, period: '', score: null as number | null, grade: '', evaluator: '', notes: '' })

async function load() {
  loading.value = true
  try {
    const [e, emp] = await Promise.all([listHrEvaluations(companyId.value, empFilter.value || undefined), listHrEmployees(companyId.value)])
    evaluations.value = e.data; employees.value = emp.data
  } finally { loading.value = false }
}
function openAdd() { editing.value = null; form.value = { employee_id: null, period: '', score: null, grade: '', evaluator: '', notes: '' }; showDialog.value = true }
...
async function handleDelete(id: number) { if (!confirm('确定删除？')) return; await deleteHrEvaluation(id); await load() }
onMounted(load)
</script>
...
<template>
  <div>
    ...
    <DataTable :value="evaluations" :loading="loading" stripedRows class="text-xs">
      ...
      <Column header="等级" style="width:60px"><template #body="{ data }"><Tag :value="data.grade" :severity="data.grade === 'A' ? 'success' : data.grade === 'B' ? 'info' : data.grade === 'C' ? 'warn' : 'danger'" /></template></Column>
    ...
    <Dialog v-model:visible="showDialog" :header="editing ? '编辑考核' : '新增考核'" :style="{ width: '450px' }">
      <div class="flex flex-col gap-3 py-4">
        ...
        <div><label class="block text-xs text-zinc-500 mb-1">考核周期</label><InputText v-model="form.period" class="w-full" placeholder="如 2026-Q1" /></div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-xs text-zinc-500 mb-1">得分</label><InputNumber v-model="form.score" class="w-full" :min-fraction-digits="1" /></div>
          <div><label class="block text-xs text-zinc-500 mb-1">等级</label><Dropdown v-model="form.grade" :options="gradeOptions" option-label="label" option-value="value" class="w-full" /></div>
        </div>
        <div><label class="block text-xs text-zinc-500 mb-1">评委</label><InputText v-model="form.evaluator" class="w-full" /></div>
