...
const rewards = ref<any[]>([]); const employees = ref<any[]>([])
const loading = ref(false); const showDialog = ref(false); const editing = ref<any>(null)
const companyId = computed(() => parseInt(localStorage.getItem('companyId') || '1'))
const form = ref({ employee_id: null as number | null, type: '奖励', date: '', description: '', amount: null as number | null, approved_by: '' })

async function load() {
  loading.value = true
  try {
    const [r, e] = await Promise.all([listHrRewards(companyId.value), listHrEmployees(companyId.value)])
    rewards.value = r.data; employees.value = e.data
  } finally { loading.value = false }
}
function openAdd() { editing.value = null; form.value = { employee_id: null, type: '奖励', date: '', description: '', amount: null, approved_by: '' }; showDialog.value = true }
...
async function handleDelete(id: number) { if (!confirm('确定删除？')) return; await deleteHrReward(id); await load() }
onMounted(load)
</script>
...
<template>
  <div>
    ...
    <DataTable :value="rewards" :loading="loading" stripedRows class="text-xs">
      ...
      <Column header="类型" style="width:70px"><template #body="{ data }"><Tag :value="data.type" :severity="data.type === '奖励' ? 'success' : 'danger'" /></template></Column>
      <Column field="date" header="日期" style="width:100px" />
      <Column field="description" header="描述" />
      <Column field="amount" header="金额" style="width:100px"><template #body="{ data }">{{ data.amount?.toLocaleString() }}</template></Column>
    ...
    <Dialog v-model:visible="showDialog" :header="editing ? '编辑记录' : '新增记录'" :style="{ width: '450px' }">
      <div class="flex flex-col gap-3 py-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-xs text-zinc-500 mb-1">类型</label><Dropdown v-model="form.type" :options="[{label:'奖励',value:'奖励'},{label:'惩罚',value:'惩罚'}]" option-label="label" option-value="value" class="w-full" /></div>
          <div><label class="block text-xs text-zinc-500 mb-1">日期</label><InputText v-model="form.date" type="date" class="w-full" /></div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-xs text-zinc-500 mb-1">金额</label><InputNumber v-model="form.amount" class="w-full" mode="currency" currency="CNY" /></div>
          <div><label class="block text-xs text-zinc-500 mb-1">批准人</label><InputText v-model="form.approved_by" class="w-full" /></div>
        </div>
        <div><label class="block text-xs text-zinc-500 mb-1">描述</label><Textarea v-model="form.description" rows="2" class="w-full" /></div>
