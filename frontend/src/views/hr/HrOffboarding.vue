...
import { listHrOffboarding, createHrOffboarding, updateHrOffboarding, deleteHrOffboarding, listHrEmployees } from '@/api'

const records = ref<any[]>([]); const employees = ref<any[]>([])
...
const form = ref({ employee_id: null as number | null, apply_date: '', last_day: '', reason: '', handover_to: '', status: '申请', notes: '' })

async function load() {
  loading.value = true
  try {
    const [o, e] = await Promise.all([listHrOffboarding(companyId.value), listHrEmployees(companyId.value)])
    records.value = o.data; employees.value = e.data
  } finally { loading.value = false }
}
function openAdd() { editing.value = null; form.value = { employee_id: null, apply_date: '', last_day: '', reason: '', handover_to: '', status: '申请', notes: '' }; showDialog.value = true }
...
async function handleDelete(id: number) { if (!confirm('确定删除？')) return; await deleteHrOffboarding(id); await load() }
onMounted(load)
</script>
...
<template>
  <div>
    ...
    <DataTable :value="records" :loading="loading" stripedRows class="text-xs">
      ...
      <Column header="状态" style="width:80px"><template #body="{ data }"><Tag :value="data.status" :severity="data.status === '已离职' ? 'danger' : data.status === '已批准' ? 'success' : 'info'" /></template></Column>
    ...
    <Dialog v-model:visible="showDialog" :header="editing ? '编辑离职' : '新增离职'" :style="{ width: '480px' }">
      <div class="flex flex-col gap-3 py-4">
        <div><label class="block text-xs text-zinc-500 mb-1">员工</label><Dropdown v-model="form.employee_id" :options="editing ? employees : activeEmployees" option-label="name" option-value="id" class="w-full" /></div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-xs text-zinc-500 mb-1">申请日期</label><InputText v-model="form.apply_date" type="date" class="w-full" /></div>
          <div><label class="block text-xs text-zinc-500 mb-1">最后工作日</label><InputText v-model="form.last_day" type="date" class="w-full" /></div>
        </div>
        <div><label class="block text-xs text-zinc-500 mb-1">离职原因</label><Textarea v-model="form.reason" rows="2" class="w-full" /></div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-xs text-zinc-500 mb-1">交接人</label><InputText v-model="form.handover_to" class="w-full" /></div>
