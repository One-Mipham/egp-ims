...
async function runBatch() {
  ...
  toast.add({
    severity: 'success',
    summary: `批量计提完成：成功${success.length}条，跳过${failed.length}条`,
    life: 4000,
  })
    ...
    <table class="w-full text-sm border-collapse">
      <thead>
        <tr class="bg-zinc-100 text-left">
          <th class="p-2 border">资产</th>
          <th class="p-2 border">期间</th>
          <th class="p-2 border text-right">本期折旧</th>
      ...
      <div class="bg-white rounded-lg w-[480px] p-6">
        ...
        <div class="space-y-3">
          <div>
            ...
            <select
              v-model.number="form.fixed_asset_id"
              @change="onAssetChange"
              class="w-full border rounded px-2 py-1.5 text-sm"
            >
              <option :value="null" disabled>-- 请选择 --</option>
              <option
                v-for="a in assets.filter((x: any) => x.status === '使用中' || x.status === '闲置')"
                :key="a.id"
                :value="a.id"
              >
                {{ a.asset_code }} {{ a.name }} (净值 {{ (a.net_value || 0).toLocaleString() }})
              </option>
            </select>
          </div>
          <div v-if="selectedAsset" class="bg-zinc-50 rounded p-3 text-xs space-y-1">
            ...
            <div class="flex justify-between">
              <span class="text-zinc-500">残值</span
              ><span>{{ (selectedAsset.residual_value || 0).toLocaleString() }}</span>
            </div>
          </div>
          <div>
            <label class="text-xs text-zinc-500">折旧期间</label
            ><input v-model="form.period" placeholder="YYYY-MM" class="w-full border rounded px-2 py-1.5 text-sm" />
          </div>
          <div>
            <label class="text-xs text-zinc-500">折旧金额</label
            ><input
              type="number"
    ...
    <div v-if="batchDialogVisible" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg w-[400px] p-6">
        ...
        <p class="text-xs text-zinc-500 mb-3">
          将为所有"使用中"和"闲置"状态的资产计提折旧。已在该期间计提过的资产将自动跳过。
        </p>
        <div class="space-y-3">
          <div>
            <label class="text-xs text-zinc-500">折旧期间</label
            ><input v-model="batchPeriod" placeholder="YYYY-MM" class="w-full border rounded px-2 py-1.5 text-sm" />
          </div>
        </div>
        <div class="flex justify-end gap-2 mt-4">
