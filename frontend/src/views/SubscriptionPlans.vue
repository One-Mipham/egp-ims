...
const displayPlans = computed(() => {
  ...
  const cyclePlans = plans.value.filter(p =>
...
function getModuleLabel(m: string) { return t(`modules.${m}`) || m }
...
function selectPlan(plan: SubscriptionPlan) {
  ...
  localStorage.setItem('selectedPlan', JSON.stringify({ slug: plan.slug, name: plan.name, price: plan.price_cny, billing_cycle: plan.billing_cycle }))
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="text-center mb-10">
        ...
        <div class="inline-flex items-center bg-stone-100 rounded-lg p-1 mt-6">
          ...
          >{{ t('subscription.annual') }} <span class="text-xs text-emerald-600 ml-1">{{ t('subscription.savePercent') }}</span></button>
        </div>
      </div>

      <!-- Plan cards -->
      <div class="grid md:grid-cols-3 gap-6 mb-12">
        <div v-for="plan in displayPlans.cyclePlans" :key="plan.slug"
          ...
          <div class="flex-1">
            <div class="flex items-center justify-between mb-2">
              ...
              <Badge v-if="plan.slug === 'pro' || plan.slug === 'pro-annual'" :value="t('subscription.recommended')" severity="info" />
            </div>
            ...
            <div class="mb-6">
              ...
              <span class="text-sm text-stone-400 ml-1">{{ selectedCycle === 'monthly' ? t('subscription.perMonth') : t('subscription.perYear') }}</span>
            </div>
          ...
          <Button :label="plan.slug.includes('pro') ? t('subscription.subscribeNow') : t('subscription.selectPlan')"
                  icon="pi pi-arrow-right" iconPos="right"
                  class="w-full" :severity="plan.slug.includes('pro') ? undefined : 'secondary'"
      ...
      <div v-if="displayPlans.lifetime.length" class="max-w-3xl mx-auto">
        ...
        <div v-for="plan in displayPlans.lifetime" :key="plan.slug"
          ...
          <div class="text-right">
            ...
            <Button :label="t('subscription.inquire')" severity="secondary" size="small" class="mt-2"
                    @click="selectPlan(plan)" />
          </div>
        </div>
      </div>
      ...
      <div class="text-center mt-8">
        <Button :label="t('subscription.startTrial')" icon="pi pi-play" size="large" rounded
                :loading="loading" @click="startTrial" />
      </div>
    </div>
  </div>
</template>
