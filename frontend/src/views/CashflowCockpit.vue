...
const manualKeys = [
  'openingBalance','salesCollection','investmentDiv','otherInflow',
  'purchaseCost','marketingExp','rdExp','adminExp','financeExp',
  'equityInv','officeInv','landInv','constructionInv','otherOutflow',
  'equityFin','debtFin',
] as const
...
const yearData = reactive<Record<ManualKey, (number | null)[]>>({
  openingBalance: [null, null, null], salesCollection: [null, null, null],
  investmentDiv: [null, null, null], otherInflow: [null, null, null],
  purchaseCost: [null, null, null], marketingExp: [null, null, null],
  rdExp: [null, null, null], adminExp: [null, null, null],
  financeExp: [null, null, null], equityInv: [null, null, null],
  officeInv: [null, null, null], landInv: [null, null, null],
  constructionInv: [null, null, null], otherOutflow: [null, null, null],
  equityFin: [null, null, null], debtFin: [null, null, null],
})
...
const outflowTotal = computed(() =>
  data.purchaseCost.map((_, i) => {
    const vals = [
      data.purchaseCost[i], data.marketingExp[i], data.rdExp[i],
      data.adminExp[i], data.financeExp[i],
...
const financingTotal = computed(() =>
  data.equityFin.map((_, i) => {
    const e = data.equityFin[i]; const d = data.debtFin[i]
...
const cashflowRows = computed<TableRow[]>(() => [
  { seq: '—',  item: '期初余额',         type: 'manual',              bold: true,  bgClass: 'bg-stone-50', getValue: i => effectiveOpening.value[i], inputKey: 'openingBalance' },
  { seq: '一', item: '资金流入',         type: 'header',                            bgClass: 'bg-emerald-50', getValue: () => null },
  { seq: '1',  item: '销售回款',         type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.salesCollection[i], inputKey: 'salesCollection' },
  { seq: '2',  item: '投资变现与分红',   type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.investmentDiv[i], inputKey: 'investmentDiv' },
  { seq: '3',  item: '其他',             type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.otherInflow[i], inputKey: 'otherInflow' },
  { seq: '',   item: '实际回款合计',     type: 'auto',   bold: true,  bgClass: 'bg-emerald-50', getValue: i => inflowTotal.value[i] },
  { seq: '二', item: '资金流出',         type: 'header',                            bgClass: 'bg-red-50',    getValue: () => null },
  { seq: '4',  item: '采购成本',         type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.purchaseCost[i], inputKey: 'purchaseCost' },
  { seq: '5',  item: '营销费用',         type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.marketingExp[i], inputKey: 'marketingExp' },
  { seq: '6',  item: '研发费用',         type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.rdExp[i], inputKey: 'rdExp' },
  { seq: '7',  item: '管理费用',         type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.adminExp[i], inputKey: 'adminExp' },
  { seq: '8',  item: '财务费用',         type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.financeExp[i], inputKey: 'financeExp' },
  { seq: '9',  item: '投资',             type: 'auto',   indent: true, bold: true,  bgClass: 'bg-stone-50',  getValue: i => investmentTotal.value[i] },
  { seq: '',   item: '股权',             type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.equityInv[i], inputKey: 'equityInv' },
  { seq: '',   item: '办公楼',           type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.officeInv[i], inputKey: 'officeInv' },
  { seq: '',   item: '土地',             type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.landInv[i], inputKey: 'landInv' },
  { seq: '',   item: '建安',             type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.constructionInv[i], inputKey: 'constructionInv' },
  { seq: '10', item: '其他',             type: 'manual', indent: true,              bgClass: 'bg-white',     getValue: i => data.otherOutflow[i], inputKey: 'otherOutflow' },
  { seq: '',   item: '实际支付合计',     type: 'auto',   bold: true,  bgClass: 'bg-red-50',    getValue: i => outflowTotal.value[i] },
...
const financingRows = computed<TableRow[]>(() => [
  { seq: '—', item: '股权融资', type: 'manual', bold: true, bgClass: 'bg-white',   getValue: i => data.equityFin[i], inputKey: 'equityFin' },
  { seq: '—', item: '债务融资', type: 'manual', bold: true, bgClass: 'bg-white',   getValue: i => data.debtFin[i], inputKey: 'debtFin' },
  { seq: '',  item: '融资合计', type: 'auto',   bold: true, bgClass: 'bg-stone-50', getValue: i => financingTotal.value[i] },
])
</script>
  ...
  <div class="space-y-6">
    ...
    <div class="form-card">
      ...
      <p class="text-[10px] text-stone-400 mb-2">
        月度数据手工填入；实际回款合计 = 销售回款 + 投资变现与分红 + 其他；
        投资 = 股权 + 办公楼 + 土地 + 建安；实际支付合计 = 采购成本 + 营销 + 研发 + 管理 + 财务 + 投资 + 其他；
        资金余额 = 期初余额 + 实际回款 − 实际支付。
      </p>
        ...
        <table class="data-table text-xs w-full">
          <thead>
            <tr>
              ...
              <th v-for="y in futureYears" :key="'cf'+y" class="w-[72px] text-right bg-sky-100 font-semibold">{{ y }}</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              ...
              <td v-for="mi in 12" :key="'m'+row.item+mi" class="text-right p-0">
                ...
                <template v-if="row.type === 'manual' && row.inputKey && !(row.item === '期初余额' && mi > 1)">
                  <input
                    ...
                    @input="(e: Event) => { if (row.inputKey) setValue(row.inputKey, mi-1, (e.target as HTMLInputElement).value) }"
                  />
                </template>
              ...
              <td v-for="q in 4" :key="'qc'+row.item+q" class="text-right font-number bg-amber-50/30 text-stone-600 px-1">
                <template v-if="row.type === 'header'"></template>
                <span v-else>{{ fmt(qSum(Array.from({length: 12}, (_, i) => row.getValue(i)), q)) }}</span>
              </td>
              ...
              >
                {{ row.type !== 'header' ? fmt(annualSum(
                  Array.from({length: 12}, (_, i) => row.getValue(i))
                )) : '' }}
              </td>

              <!-- Future year cells (2027-2029) -->
              <td v-for="yi in 3" :key="'cfy'+row.item+yi" class="text-right p-0"
              ...
              >
                <template v-if="row.type !== 'header' && row.inputKey">
                  <input type="number"
                    ...
                    @input="(e: Event) => { if (row.inputKey) setYearValue(row.inputKey, yi, (e.target as HTMLInputElement).value) }"
                  />
                </template>
        ...
        <table class="data-table text-xs w-full">
          <thead>
            <tr>
              ...
              <th class="w-[72px] text-right bg-stone-200 font-semibold">年度合计</th>
              <th v-for="y in futureYears" :key="'ff'+y" class="w-[72px] text-right bg-sky-100 font-semibold">{{ y }}</th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="row in financingRows"
              :key="row.item"
              :class="[row.bgClass, { 'font-semibold': row.bold }]"
            >
              <td class="text-center text-stone-400 text-[10px]">{{ row.seq }}</td>
              ...
              <!-- Quarterly cells (auto) -->
              <td v-for="q in 4" :key="'fq'+row.item+q" class="text-right font-number bg-amber-50/30 text-stone-600 px-1">
                {{ fmt(qSum(Array.from({length: 12}, (_, i) => row.getValue(i)), q)) }}
              </td>
              ...
              <!-- Future year cells -->
              <td v-for="yi in 3" :key="'ffy'+row.item+yi" class="text-right p-0"
                :class="yi === 0 ? 'bg-sky-50' : 'bg-sky-50/30'"
              >
    ...
    <div class="form-card">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <!-- 1.3 财务费用 -->
        <div class="border border-stone-200 rounded-sm p-3 bg-white">
          <label class="text-[10px] text-stone-400 mb-1 block uppercase tracking-wide">1.3 财务费用水平（年化利率 %）</label>
          <input type="number" step="0.01" placeholder="例如：4.35"
            class="w-full text-xs px-2 py-1.5 border border-stone-200 rounded-sm focus:border-amber-400 focus:outline-none font-number" />
        </div>
        <!-- 1.4 金融机构 -->
        <div class="border border-stone-200 rounded-sm p-3 bg-white">
          <label class="text-[10px] text-stone-400 mb-1 block uppercase tracking-wide">1.4 金融机构名称</label>
          <input type="text" placeholder="例如：XX银行"
            class="w-full text-xs px-2 py-1.5 border border-stone-200 rounded-sm focus:border-amber-400 focus:outline-none" />
        </div>
      </div>
    </div>
    ...
    <div class="form-card">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div class="border border-stone-200 rounded-sm p-3 bg-white md:col-span-2">
          ...
          <div class="flex gap-2">
            <input type="text" placeholder="律所名称"
              class="flex-1 text-xs px-2 py-1.5 border border-stone-200 rounded-sm focus:border-amber-400 focus:outline-none" />
            <div class="flex items-center gap-1 bg-stone-50 rounded-sm px-2 border border-stone-200">
        ...
        <div class="border border-stone-200 rounded-sm p-3 bg-white md:col-span-2">
          ...
          <div class="flex gap-2">
            <input type="text" placeholder="会所名称"
              class="flex-1 text-xs px-2 py-1.5 border border-stone-200 rounded-sm focus:border-amber-400 focus:outline-none" />
            <div class="flex items-center gap-1 bg-stone-50 rounded-sm px-2 border border-stone-200">
              <span class="text-[10px] text-stone-400 whitespace-nowrap">费用 万元</span>
              <input type="number" placeholder="—"
