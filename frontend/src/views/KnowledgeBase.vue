  ...
  <div class="flex h-full">
    ...
    <div class="w-72 border-r border-dashed border-zinc-200 flex flex-col shrink-0">
      <div class="p-3 border-b border-dashed border-zinc-200">
        <input v-model="searchText" @input="onSearch" placeholder="搜索标题/正文..." class="w-full border rounded px-2 py-1.5 text-xs" />
      </div>
      <div class="overflow-auto flex-1">
        <div class="flex items-center px-3 py-1 border-b border-dashed border-zinc-200 text-[10px] text-zinc-400">
          <input type="checkbox" :checked="selectedIds.size === articles.length && articles.length > 0" @change="toggleSelectAll" class="mr-2" />
          <span>全选</span>
        </div>
        <div
        ...
        >
          <input type="checkbox" :checked="selectedIds.has(article.id)" @click.stop @change="toggleSelect(article.id)" class="mr-2 mt-0.5 shrink-0" />
          <div class="flex-1 min-w-0" @click="selectArticle(article)">
            ...
            <div class="flex items-center gap-2 mt-0.5">
              <span
                ...
                :class="article.status === 'published' ? 'bg-green-100 text-green-700' : article.status === 'archived' ? 'bg-zinc-200 text-zinc-500' : 'bg-amber-100 text-amber-700'"
              >{{ article.status === 'published' ? '已发布' : article.status === 'archived' ? '已归档' : '草稿' }}</span>
              <span class="text-[10px] text-zinc-300">v{{ article.version }}</span>
            </div>
          </div>
        </div>
        <div v-if="articles.length === 0" class="p-6 text-center text-xs text-zinc-400">
          暂无文章
        </div>
        <div class="flex items-center justify-between px-3 py-2 border-t border-dashed border-zinc-200 text-[10px]">
          ...
          <div class="flex gap-1">
            <button @click="page = Math.max(1, page - 1); loadArticles()" :disabled="page <= 1" class="px-2 py-0.5 border rounded disabled:opacity-30">上一页</button>
            <button @click="page = page + 1; loadArticles()" class="px-2 py-0.5 border rounded">下一页</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <template v-if="selectedArticle">
        <div class="flex items-center justify-between px-4 py-2 border-b bg-white">
          ...
          <div class="flex items-center gap-2">
            <button @click="togglePublish" class="px-3 py-1 text-xs rounded border" :class="selectedArticle.status === 'published' ? 'text-amber-600 border-amber-300' : 'text-green-600 border-green-300'">
              {{ selectedArticle.status === 'published' ? '取消发布' : '发布' }}
            </button>
            <button @click="openEdit" class="px-3 py-1 text-xs rounded border text-blue-600 border-blue-300">编辑</button>
        ...
        <div class="flex-1 overflow-auto p-6">
          ...
          <div v-if="selectedArticle.tags" class="flex gap-1 mt-6 pt-4 border-t">
            <span v-for="t in selectedArticle.tags.split(',').filter(Boolean)" :key="t" class="text-[10px] bg-zinc-100 text-zinc-500 px-2 py-0.5 rounded">{{ t }}</span>
          </div>
        </div>
      </template>
    ...
    <div v-if="editorVisible" class="fixed inset-0 bg-black/40 flex items-start justify-center z-50 pt-10">
      <div class="bg-white rounded-lg w-[800px] max-h-[85vh] flex flex-col">
        ...
        <div class="p-4 space-y-3 overflow-auto flex-1">
          <div><label class="text-xs text-zinc-500">标题</label><input v-model="form.title" class="w-full border rounded px-2 py-1.5 text-sm" /></div>
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-1 mb-1">
              <span v-for="t in form.tags" :key="t" class="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded flex items-center gap-1">
                {{ t }} <button @click="removeTag(t)" class="text-blue-400 hover:text-red-500">&times;</button>
              </span>
            </div>
            <div class="flex gap-1">
              <input v-model="tagInput" @keydown.enter.prevent="addTag" placeholder="输入标签后回车" class="flex-1 border rounded px-2 py-1 text-xs" />
          ...
          <div>
            ...
            <textarea v-model="form.content_md" rows="14" class="w-full border rounded px-2 py-1.5 text-sm font-mono" placeholder="支持 Markdown 格式..."></textarea>
          </div>
        </div>
...
<style scoped>
.prose :deep(h1) { font-size: 1.5rem; font-weight: 700; margin-top: 1.5rem; margin-bottom: 0.75rem; }
.prose :deep(h2) { font-size: 1.25rem; font-weight: 600; margin-top: 1.25rem; margin-bottom: 0.5rem; }
.prose :deep(h3) { font-size: 1.1rem; font-weight: 600; margin-top: 1rem; margin-bottom: 0.5rem; }
...
.prose :deep(code) { background: #f4f4f5; padding: 2px 4px; border-radius: 3px; font-size: 0.85em; }
.prose :deep(pre) { background: #1e1e1e; color: #d4d4d4; padding: 1rem; border-radius: 6px; overflow-x: auto; margin-bottom: 0.75rem; }
.prose :deep(pre code) { background: none; padding: 0; color: inherit; }
.prose :deep(blockquote) { border-left: 3px solid #e4e4e7; padding-left: 1rem; color: #71717a; margin-bottom: 0.75rem; }
...
.prose :deep(th), .prose :deep(td) { border: 1px solid #e4e4e7; padding: 0.5rem; text-align: left; font-size: 0.875rem; }
.prose :deep(th) { background: #f4f4f5; font-weight: 600; }
.prose :deep(hr) { border: none; border-top: 1px solid #e4e4e7; margin: 1rem 0; }
