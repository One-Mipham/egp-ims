...
import { listReceivables, createReceivable, updateReceivable, deleteReceivable, listReceivableCounterparties } from '../../api'
...
async function remove(id: number) {
  if (confirm('确定删除？')) { await deleteReceivable(id); toast.add({ severity: 'success', summary: '已删除', life: 2000 }); await load() }
}

function exportCSV() {
  const header = ['客户名称', '发票号', '发票日期', '金额', '已收金额', '余额', '到期日', '账龄(天)', '状态', '备注']
  const rows = items.value.map((i: any) => [i.customer_name, i.invoice_no, i.invoice_date || '', i.amount, i.received_amount, i.balance, i.due_date || '', i.aging_days, i.status, i.notes || ''])
  const csv = [header.join(','), ...rows.map((r: any[]) => r.map((c: any) => `"${String(c ?? '').replace(/"/g, '""')}"`).join(','))].join('\n')
  const blob = new Blob(['﻿' + csv], { type: 'text/csv;charset=utf-8;' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url; a.download = '应收发票.csv'; a.click()
...
<template>
  <div class="p-4">
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-lg font-bold">应收发票</h1>
    ...
    <table class="w-full text-sm border-collapse">
      ...
      <tbody>
        <tr v-for="item in items" :key="item.id" class="hover:bg-zinc-50">
          ...
          <td class="p-2 border"><span :class="item.status === '已收款' ? 'text-green-600' : item.status === '坏账' ? 'text-red-500' : 'text-amber-600'">{{ item.status }}</span></td>
          <td class="p-2 border">
