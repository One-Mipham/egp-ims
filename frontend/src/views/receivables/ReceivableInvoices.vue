...
<template>
  <div class="p-4">
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-lg font-bold">{{ t('receivables.invoices') }}</h1>
      ...
      <tbody>
        <tr v-for="item in items" :key="item.id" class="hover:bg-zinc-50">
          ...
          <td
            ...
            :class="(item.balance || 0) > 0 ? 'text-red-600' : 'text-green-600'"
          >
          ...
          <td class="p-2 border">
            <span
              :class="
                item.status === '已收款' ? 'text-green-600' : item.status === '坏账' ? 'text-red-500' : 'text-amber-600'
              "
              >{{ item.status }}</span
            >
          </td>
