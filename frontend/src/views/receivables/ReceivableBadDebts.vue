...
const totalBadDebt = computed(() =>
  overdueItems.value.reduce((s: number, i: any) => s + (i.balance || 0), 0)
)
...
async function markBadDebt(row: any) {
  if (!confirm(`确定将 "${row.customer_name} - ${row.invoice_no}" 标记为坏账？余额 ${(row.balance || 0).toLocaleString()} 将计提损失。`)) return
...
<template>
  <div class="p-4">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="text-sm"><span class="text-zinc-500">坏账余额合计：</span><span class="text-red-600 font-bold text-lg">{{ totalBadDebt.toLocaleString() }}</span></div>
    </div>

    <table class="w-full text-sm border-collapse">
      <thead>
        <tr class="bg-zinc-100 text-left">
          <th class="p-2 border">客户</th><th class="p-2 border">发票号</th><th class="p-2 border text-right">金额</th>
          <th class="p-2 border text-right">余额</th><th class="p-2 border text-right">账龄(天)</th>
          <th class="p-2 border">状态</th><th class="p-2 border">到期日</th><th class="p-2 border">操作</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="item in overdueItems" :key="item.id" class="hover:bg-zinc-50" :class="item.status === '坏账' ? 'bg-red-50' : ''">
          ...
          <td class="p-2 border"><span :class="item.status === '坏账' ? 'text-red-600 font-bold' : 'text-amber-600'">{{ item.status }}</span></td>
          <td class="p-2 border text-xs">{{ item.due_date }}</td>
          <td class="p-2 border">
            <button v-if="item.status !== '坏账'" @click="markBadDebt(item)" class="text-red-600 text-xs">标记坏账</button>
