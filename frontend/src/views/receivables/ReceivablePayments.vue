...
const emptyForm = () => ({
  company_id: companyId, receivable_id: null as number | null,
...
async function load() {
  ...
  const [r1, r2] = await Promise.all([listReceivablePayments(companyId, params), listReceivables(companyId, { limit: 500 })])
  items.value = r1.data
...
<template>
  <div class="p-4">
    <div class="flex items-center justify-between mb-4">
      ...
      <button @click="openCreate" class="px-4 py-2 bg-green-600 text-white rounded text-sm hover:bg-green-700">+ 登记收款</button>
    </div>

    <table class="w-full text-sm border-collapse">
      <thead>
        <tr class="bg-zinc-100 text-left">
          <th class="p-2 border">应收发票</th><th class="p-2 border">收款日期</th>
        ...
        <div class="space-y-3">
          <div>
            ...
            <select v-model.number="form.receivable_id" class="w-full border rounded px-2 py-1.5 text-sm">
              ...
              <option v-for="r in receivables" :key="r.id" :value="r.id">{{ r.customer_name }} · {{ r.invoice_no }} (余额 {{ (r.balance || 0).toLocaleString() }})</option>
            </select>
          </div>
          <div><label class="text-xs text-zinc-500">收款日期</label><input type="date" v-model="form.payment_date" class="w-full border rounded px-2 py-1.5 text-sm" /></div>
          ...
          <div>
            ...
            <select v-model="form.payment_method" class="w-full border rounded px-2 py-1.5 text-sm">
              <option>银行转账</option><option>现金</option><option>承兑汇票</option><option>微信/支付宝</option>
            </select>
          </div>
