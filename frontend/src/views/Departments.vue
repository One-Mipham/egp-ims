...
async function handleImport() {
  ...
  if (!rows.length) { importResult.value = '未解析到有效数据，请检查格式'; return }
  ...
  <div>
    <div class="flex justify-between items-center mb-4">
      <div class="flex gap-2">
        <Button label="导入" icon="pi pi-upload" severity="secondary" @click="showImportDialog = true; importResult = ''" />
    ...
    <div class="bg-white rounded-sm border border-stone-200 overflow-x-auto max-w-fit min-w-full">
      <DataTable :value="departments" :loading="loading" stripedRows class="shadow-sm" tableStyle="min-width: auto">
        ...
        <template #body="{ data }">
          <Button v-if="data.is_active" label="停用" text severity="danger" size="small" @click="handleDelete(data.id)" />
        </template>
      </Column>
    </DataTable>
    </div>
    ...
    <Dialog v-model:visible="showImportDialog" header="导入部门" :style="{ width: '600px' }">
      <div class="flex flex-col gap-4 py-4">
        ...
        <textarea v-model="importText" rows="12" class="w-full border border-zinc-300 rounded-sm p-2 text-sm font-mono focus:ring-1 focus:ring-zinc-400 outline-none" placeholder="001	董事会" />
        <p v-if="importResult" :class="importResult.includes('错误') || importResult.includes('失败') ? 'text-red-700' : 'text-emerald-600'" class="text-sm tracking-wide">{{ importResult }}</p>
