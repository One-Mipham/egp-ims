...
const paymentMethods = computed(() => {
  if (locale.value === 'en-US') {
    return [
      { value: 'bank_transfer', label: t('subscription.checkout.bankTransfer'), icon: 'pi pi-building', desc: t('subscription.checkout.bankDesc') },
      { value: 'paypal', label: t('subscription.checkout.paypal'), icon: 'pi pi-paypal', desc: t('subscription.checkout.paypalDesc') },
      { value: 'stripe', label: t('subscription.checkout.stripe'), icon: 'pi pi-credit-card', desc: t('subscription.checkout.stripeDesc') },
    ]
  }
  return [
    { value: 'bank_transfer', label: t('subscription.checkout.bankTransfer'), icon: 'pi pi-building', desc: t('subscription.checkout.bankDesc') },
    { value: 'alipay', label: t('subscription.checkout.alipay'), icon: 'pi pi-mobile', desc: t('subscription.checkout.alipayDesc') },
    { value: 'wechat', label: t('subscription.checkout.wechat'), icon: 'pi pi-comments', desc: t('subscription.checkout.wechatDesc') },
  ]
})
...
const displayPrice = computed(() => {
  ...
  if (locale.value === 'en-US') {
    return '$' + (selectedPlan.value.price / 7.2).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })
  }
  ...
  <div class="min-h-screen bg-stone-50 py-12 px-4">
    <div class="max-w-lg mx-auto">
      ...
      <div class="bg-white rounded-xl border border-stone-200 p-6 mb-6">
        ...
        <div v-for="pm in paymentMethods" :key="pm.value"
      ...
      <div v-if="showBankInfo" class="bg-stone-50 rounded-xl border border-stone-200 p-6 mb-6">
        ...
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-mono text-stone-700">{{ locale === 'en-US' ? 'ICBC Beijing Haidian Branch' : '中国工商银行北京海淀支行' }}</span>
          </div>
          <div class="flex justify-between">
      ...
      <div v-if="showBankInfo" class="bg-white rounded-xl border border-stone-200 p-6 mb-6">
        ...
        <div class="border-2 border-dashed border-stone-300 rounded-lg p-6 text-center hover:border-indigo-400 transition-colors cursor-pointer"
             @click="($refs.fileInput as any)?.click()">
          <input ref="fileInput" type="file" accept="image/*,.pdf" class="hidden" @change="(e: any) => onFileSelect({ files: e.target.files })" />
          <template v-if="!receiptFile">
            ...
            <p class="text-xs text-stone-400 mt-1">{{ locale === 'en-US' ? 'PNG, JPG or PDF, max 5MB' : '支持 PNG、JPG、PDF，最大 5MB' }}</p>
          </template>
          <template v-else>
      ...
      <Button :label="t('subscription.checkout.confirmPay')" icon="pi pi-check" class="w-full" size="large" @click="confirmPayment" />
      <Button :label="t('subscription.checkout.backToPlans')" severity="secondary" text class="w-full mt-2" @click="router.push('/subscription/plans')" />
    </div>
  </div>
</template>
