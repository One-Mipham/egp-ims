  ...
  <div>
    <div class="flex justify-between items-center mb-4">
      <div class="flex gap-2 items-center">
        <input v-model="currentPeriod" type="month" class="px-3 py-2 border border-zinc-300 rounded-sm text-sm focus:ring-1 focus:ring-zinc-400 outline-none" @change="loadReport" />
        <Button label="刷新" icon="pi pi-refresh" text @click="loadReport" :loading="loading" />
      </div>
    </div>

    <div class="flex gap-1 mb-6">
      <Button label="资产负债表" :severity="activeReport === 'balance' ? 'danger' : 'secondary'" :text="activeReport !== 'balance'" @click="activeReport = 'balance'" />
      <Button label="利润表" :severity="activeReport === 'income' ? 'danger' : 'secondary'" :text="activeReport !== 'income'" @click="activeReport = 'income'" />
      <Button label="现金流量表" :severity="activeReport === 'cashflow' ? 'danger' : 'secondary'" :text="activeReport !== 'cashflow'" @click="activeReport = 'cashflow'" />
    </div>
    ...
    <div v-if="activeReport === 'balance' && balanceData" class="print-area">
      <div class="report-container bg-white shadow-sm px-12 pt-6 pb-4 max-w-[96%] mx-auto">
        ...
        <table class="data-table border-collapse border border-stone-300">
          ...
          <tbody>
            <tr v-for="i in Math.max(balanceData.left_items?.length || 0, balanceData.right_items?.length || 0)" :key="i">
              <td v-if="balanceData.left_items?.[i-1]" class="py-1.5 px-2 border border-stone-200"
                  :class="{ 'font-bold bg-stone-50': balanceData.left_items[i-1].name.includes('合计') || balanceData.left_items[i-1].name.includes('总计') }">
                {{ balanceData.left_items[i-1].name }}
              </td>
              <td v-else class="py-1.5 px-2 border border-stone-200"></td>
              <td v-if="balanceData.left_items?.[i-1]" class="report-number py-1.5 px-2 border border-stone-200"
                  :class="{ 'font-bold bg-stone-50': balanceData.left_items[i-1].name.includes('合计') || balanceData.left_items[i-1].name.includes('总计') }">
              ...
              <td v-else class="py-1.5 px-2 border border-stone-200"></td>
              <td v-if="balanceData.right_items?.[i-1]" class="py-1.5 px-2 border border-stone-200"
                  :class="{ 'font-bold bg-stone-50': balanceData.right_items[i-1].name.includes('合计') || balanceData.right_items[i-1].name.includes('总计') }">
                {{ balanceData.right_items[i-1].name }}
              </td>
              <td v-else class="py-1.5 px-2 border border-stone-200"></td>
              <td v-if="balanceData.right_items?.[i-1]" class="report-number py-1.5 px-2 border border-stone-200"
                  :class="{ 'font-bold bg-stone-50': balanceData.right_items[i-1].name.includes('合计') || balanceData.right_items[i-1].name.includes('总计') }">
        ...
        <table class="data-table border-collapse border border-stone-300">
          <thead>
            <tr class="border-b-2 border-stone-400 bg-stone-50">
              ...
              <th class="text-right py-1.5 px-2 font-bold border border-stone-200 report-number" style="width:17%">本月金额</th>
              <th class="text-right py-1.5 px-2 font-bold border border-stone-200 report-number" style="width:17%">本年累计金额</th>
              <th class="text-right py-1.5 px-2 font-bold border border-stone-200 report-number" style="width:17%">上年同期累计</th>
            </tr>
          </thead>
          <tbody>
...
@media print {
  ...
  .report-container { margin: 0 auto; page-break-inside: avoid; }
}
</style>
