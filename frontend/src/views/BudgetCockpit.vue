...
const futureYears = computed(() => [
  `${currentYear + 1}年`, `${currentYear + 2}年`, `${currentYear + 3}年`,
])
...
const yearData = reactive<Record<typeof manualKeys[number], (number | null)[]>>({
  revenue: [null, null, null], cost: [null, null, null],
  operatingExp: [null, null, null], adminExp: [null, null, null],
  financeExp: [null, null, null], otherIncExp: [null, null, null],
...
const yearGrossMargin = computed(() =>
  yearGrossProfit.value.map((v, i) => (v != null && yearData.revenue[i] != null && yearData.revenue[i] !== 0)
    ? parseFloat((v / yearData.revenue[i]! * 100).toFixed(1)) : null)
)
const yearPeriodExp = computed(() =>
  yearData.revenue.map((_, i) => {
    const o = yearData.operatingExp[i]; const a = yearData.adminExp[i]; const f = yearData.financeExp[i]
...
const yearNetProfit = computed(() =>
  yearPretaxProfit.value.map((v, i) => (v != null && yearData.incomeTax[i] != null) ? v - yearData.incomeTax[i]! : null)
)
...
function getYearValue(label: string, yi: number): number | null {
  switch (label) {
    case '收入': return getAutoRevenue(yi)  // Always auto-computed from growth rate
    case '成本': return yearData.cost[yi]
    case '毛利润': return yearGrossProfit.value[yi]
    case '毛利率': return yearGrossMargin.value[yi]
    case '经营费用': return yearData.operatingExp[yi]
    case '管理费用': return yearData.adminExp[yi]
    case '财务费用': return yearData.financeExp[yi]
    case '期间费用合计': return yearPeriodExp.value[yi]
    case '营业外收支': return yearData.otherIncExp[yi]
    case '税前利润': return yearPretaxProfit.value[yi]
    case '所得税': return yearData.incomeTax[yi]
    case '税后利润': return yearNetProfit.value[yi]
    default: return null
  }
}
function getYearInputKey(label: string): typeof manualKeys[number] | null {
  switch (label) {
    case '收入': return null  // Revenue is auto-computed, not manual
    case '成本': return 'cost'
    case '经营费用': return 'operatingExp'
    case '管理费用': return 'adminExp'
    case '财务费用': return 'financeExp'
    case '营业外收支': return 'otherIncExp'
    case '所得税': return 'incomeTax'
    default: return null
  }
}
...
const grossMargin = computed(() =>
  grossProfit.value.map((v, i) => (v != null && data.revenue[i] != null && data.revenue[i] !== 0)
    ? parseFloat((v / data.revenue[i]! * 100).toFixed(1)) : null)
)
const periodExp = computed(() =>
  data.revenue.map((_, i) => {
    const o = data.operatingExp[i]; const a = data.adminExp[i]; const f = data.financeExp[i]
    if (o == null && a == null && f == null) return null
    return (o ?? 0) + (a ?? 0) + (f ?? 0)
  })
)
...
function buildBudgetItems(): { account_code: string; month: string; amount: number }[] {
  ...
  const rowMap: Record<string, (number | null)[]> = {
    revenue: data.revenue, cost: data.cost, operatingExp: data.operatingExp,
    adminExp: data.adminExp, financeExp: data.financeExp,
    otherIncExp: data.otherIncExp, incomeTax: data.incomeTax,
  }
...
<template>
  <div class="space-y-6">
        ...
        <table class="data-table text-xs w-full">
          <thead>
            <tr>
              ...
              <th v-for="y in futureYears" :key="y" class="w-[80px] text-right bg-sky-100 font-semibold whitespace-nowrap">{{ y }}规划</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
                  ...
                  <input
                    ...
                    @input="(e: Event) => {
                      ...
                      const map: Record<string, (typeof manualKeys)[number]> = {
                        '收入': 'revenue', '成本': 'cost', '经营费用': 'operatingExp',
                        '管理费用': 'adminExp', '财务费用': 'financeExp',
                        '营业外收支': 'otherIncExp', '所得税': 'incomeTax',
                      }
              ...
              <!-- Q1-Q4 subtotals -->
              <td v-for="q in 4" :key="'q'+row.label+q" class="text-right font-number bg-amber-50/30 text-stone-600 px-1">
                {{ row.format ? row.format(qSum(Array.from({length: 12}, (_, i) => getRowValues(row, i)), q)) : '' }}
              </td>
              ...
              <td v-for="yi in 3" :key="'fy'+row.label+yi" class="text-right p-0"
    ...
    <div class="form-card">
      ...
      <p class="text-[10px] text-stone-400 mb-2">
        收入、净利润实际数据从会计系统调用；完成率 = 实际 / 预算 × 100%。
      </p>
