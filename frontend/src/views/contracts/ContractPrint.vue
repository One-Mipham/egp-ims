...
<template>
  ...
  <div v-else-if="contract" class="max-w-4xl mx-auto p-6">
    ...
    <div
      class="bg-white border shadow-lg p-10 print:shadow-none print:border-none print:p-0"
    ...
    >
      ...
      <p v-if="contract.legal_basis" class="mb-4 text-justify indent-8">
        ...
        甲乙双方在平等、自愿、公平和诚实信用的基础上，经协商一致，就{{
          contract.subject || '相关事宜'
        }}达成如下协议，以资共同遵守：
      </p>
      ...
      <div class="mb-4">
        ...
        <p class="indent-8 text-justify">
          {{ contract.subject || '（双方应根据实际情况填写合同标的的具体内容、范围及要求。）' }}
        </p>
      </div>
      ...
      <div class="mb-4">
        ...
        <p class="indent-8">
          合同总金额：<strong>¥{{ contract.amount?.toLocaleString() || '0' }}</strong
          >（人民币）
        </p>
        <div v-if="contract.payment_terms" class="indent-8 text-justify whitespace-pre-wrap">
      ...
      <div class="mb-4">
        ...
        <p class="indent-8">
          本合同自 <strong>{{ fmtDate(contract.start_date) }}</strong> 起生效， 至
          <strong>{{ fmtDate(contract.end_date) }}</strong> 止。
        </p>
      </div>
      ...
      <div class="mb-4">
        ...
        <p v-if="contract.arbitration_venue" class="indent-8">
          因本合同引起的或与本合同有关的任何争议，双方应友好协商解决；协商不成的， 任何一方均有权向<strong>
            {{ contract.arbitration_venue }} </strong
          >提起诉讼/申请仲裁。
        </p>
        <p v-else class="indent-8">
          因本合同引起的或与本合同有关的任何争议，双方应友好协商解决；协商不成的， 任何一方均有权向<strong>
            合同签订地有管辖权的人民法院 </strong
          >提起诉讼。
        </p>
      </div>
