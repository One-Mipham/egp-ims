...
const PROP_TYPES = [
  { label: '商业 Commercial', value: 'commercial' },
  { label: '住宅 Residential', value: 'residential' },
  { label: '工业 Industrial', value: 'industrial' },
  { label: '土地 Land', value: 'land' },
...
async function load() {
  loading.value = true
  try {
    const r = await api.get('/investments/real-estate', {
      params: { company_id: companyId.value, ...(filterType.value ? { property_type: filterType.value } : {}) },
    })
    items.value = r.data
  } finally {
    loading.value = false
...
async function handleSave() {
  ...
  if (isEdit.value && editId.value) {
    await api.put(`/investments/real-estate/${editId.value}`, form.value)
  } else {
    await api.post('/investments/real-estate', form.value, { params: { company_id: companyId.value } })
  }
...
async function openValuations(row: any) {
  ...
  try {
    const r = await api.get(`/investments/real-estate/${row.id}/valuations`)
    valuations.value = r.data
  } catch {
    valuations.value = []
  }
...
async function saveValuation() {
  ...
  await api.post(`/investments/real-estate/${selectedAsset.value.id}/valuations`, valForm.value, {
...
<template>
  <div class="p-4">
    ...
    <DataTable :value="items" :loading="loading" stripedRows size="small" paginator :rows="10">
      ...
      >
      <Column header="出租率"
        ><template #body="{ data }">{{ data.occupancy_pct ? data.occupancy_pct + '%' : '-' }}</template></Column
      >
      <Column header="年租金"
    ...
    <Dialog v-model:visible="showDialog" :header="isEdit ? '编辑资产' : '新增资产'" :style="{ width: '520px' }" modal>
      <div class="flex flex-col gap-3 pt-2">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="text-sm text-stone-600">收购日期</label
            ><InputText v-model="form.acquisition_date" class="w-full" placeholder="YYYY-MM-DD" />
          </div>
          <div>
            <label class="text-sm text-stone-600">收购成本</label
            ><InputNumber v-model="form.acquisition_cost" mode="currency" currency="CNY" class="w-full" />
          </div>
        </div>
        <div class="grid grid-cols-3 gap-3">
          ...
          <div>
            <label class="text-sm text-stone-600">出租率%</label
            ><InputNumber v-model="form.occupancy_pct" suffix="%" class="w-full" />
          </div>
          <div>
            <label class="text-sm text-stone-600">年租金</label
            ><InputNumber v-model="form.annual_rental_income" mode="currency" currency="CNY" class="w-full" />
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
    ...
    <!-- Valuations Dialog -->
    <Dialog
      v-model:visible="showValDialog"
      :header="'估值历史: ' + (selectedAsset?.property_name || '')"
      :style="{ width: '600px' }"
      modal
    >
      <div class="flex gap-2 mb-3">
        ...
        <Dropdown
          v-model="valForm.valuation_method"
          :options="valMethods"
          optionLabel="label"
          optionValue="value"
          class="w-28"
