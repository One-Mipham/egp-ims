...
import { listUsers, createUser, updateUser, deleteUser, resetUserPassword, changeMyPassword, getMe, getUserPermissions, setUserPermissions } from '@/api'
  ...
  <div>
    <div class="flex justify-between items-center mb-4">
      <div class="flex gap-2">
        <Button label="修改我的密码" icon="pi pi-key" severity="secondary" size="small" @click="showChangeOwnDialog = true" />
    ...
    <div class="bg-white rounded-sm border border-stone-200 overflow-x-auto max-w-fit min-w-full">
      <DataTable :value="users" :loading="loading" stripedRows class="shadow-sm" tableStyle="min-width: auto">
        ...
        <Column header="最后登录" style="width:155px">
          <template #body="{ data }">
            <span class="text-xs text-stone-400">{{ data.last_login ? new Date(data.last_login).toLocaleString('zh-CN') : '—' }}</span>
          </template>
        </Column>
        <Column header="操作" style="width:230px">
        <template #body="{ data }">
          <Button v-if="data.is_active" label="停用" text severity="danger" size="small" @click="handleToggleActive(data)" />
          ...
          <Button v-if="data.is_active" label="删除" text severity="danger" size="small" @click="handleDelete(data)" />
        </template>
      </Column>
    </DataTable>
    </div>
    ...
    <Dialog v-model:visible="showAddDialog" header="新增用户" :style="{ width: '450px' }">
      <div class="flex flex-col gap-4 py-4">
        ...
        <div>
          ...
          <Dropdown v-model="newUser.role" :options="roleOptions" option-label="label" option-value="value" class="w-full" />
        </div>
    ...
    <Dialog v-model:visible="showChangeOwnDialog" header="修改我的密码" :style="{ width: '400px' }">
      <div class="flex flex-col gap-4 py-4">
        ...
        <Button label="确认修改" icon="pi pi-check" @click="handleChangeOwnPassword" :disabled="!newOwnPassword.current || !newOwnPassword.new" />
      </div>
    </Dialog>
    ...
    <Dialog v-model:visible="showPermDialog" header="用户权限管理" :style="{ width: '450px' }">
      <div class="flex flex-col gap-3 py-4">
        <p class="text-sm text-zinc-600 tracking-wide">
          为 <span class="font-medium text-zinc-800">{{ permTarget?.username }}</span>（{{ ROLE_LABELS[permTarget?.role] || permTarget?.role }}）设置细粒度权限
        </p>
