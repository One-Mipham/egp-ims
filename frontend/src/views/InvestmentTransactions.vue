...
<template>
  <div>
    ...
    <Dialog v-model:visible="showDialog" :header="isEdit ? '编辑交易' : '新增交易（自动生成凭证）'" :modal="true" class="w-[500px]">
      <div class="flex flex-col gap-3">
        <div><label class="block text-sm mb-1">持仓 *</label><Dropdown v-model="form.position_id" :options="positions" optionLabel="security_name" optionValue="id" class="w-full" filter :disabled="isEdit" /></div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-sm mb-1">交易类型 *</label><Dropdown v-model="form.transaction_type" :options="TXN_TYPES" optionLabel="label" optionValue="value" class="w-full" /></div>
        ...
        <p v-if="!isEdit" class="text-xs text-zinc-400"><i class="pi pi-info-circle mr-1" /> 保存后将自动生成会计凭证</p>
      </div>
      <template #footer>
