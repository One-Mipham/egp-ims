...
import { listInvStock, createInvStock, updateInvStock, deleteInvStock, listWarehouses, listInventoryCategories, listUnitsOfMeasure } from '../../api'
...
const emptyForm = () => ({
  company_id: companyId, product_code: '', product_name: '', category: '',
...
function exportCSV() {
  ...
  const rows = items.value.map((i: any) => [i.product_code, i.product_name, i.category, i.quantity, i.unit, i.unit_cost, i.total_cost, i.warehouse, i.min_stock])
  const csv = [header.join(','), ...rows.map((r: any[]) => r.map((c: any) => `"${String(c ?? '').replace(/"/g, '""')}"`).join(','))].join('\n')
  const blob = new Blob(['﻿' + csv], { type: 'text/csv;charset=utf-8;' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url; a.download = '库存.csv'; a.click()
  URL.revokeObjectURL(url)
}

onMounted(async () => {
  const [w, c, u] = await Promise.all([listWarehouses(companyId), listInventoryCategories(companyId), listUnitsOfMeasure(companyId)])
  warehouses.value = w.data; categories.value = c.data; units.value = u.data
  await load()
})
</script>
...
<template>
  <div class="p-4">
    ...
    <div class="flex gap-2 mb-3 flex-wrap">
      <input v-model="searchText" @input="page=1; load()" placeholder="搜索编码/名称/类别..." class="border rounded px-2 py-1 text-sm w-56" />
      <input v-model="filterCategory" @input="page=1; load()" placeholder="类别筛选" class="border rounded px-2 py-1 text-sm w-28" />
      <input v-model="filterWarehouse" @input="page=1; load()" placeholder="仓库筛选" class="border rounded px-2 py-1 text-sm w-28" />
      <label class="flex items-center gap-1 text-sm cursor-pointer">
        <input type="checkbox" v-model="lowStockOnly" @change="page=1; load()" />
    ...
    <table class="w-full text-sm border-collapse">
      ...
      <tbody>
        <tr v-for="item in items" :key="item.id" class="hover:bg-zinc-50" :class="(item.quantity || 0) <= (item.min_stock || 0) ? 'bg-red-50' : ''">
          ...
          <td class="p-2 border text-right" :class="(item.quantity || 0) <= (item.min_stock || 0) ? 'text-red-600 font-bold' : ''">
            {{ item.quantity }} {{ item.unit }}
            <span v-if="(item.quantity || 0) <= (item.min_stock || 0)" class="text-xs text-red-500 ml-1">⚠低于最低库存</span>
          </td>
    ...
    <div v-if="dialogVisible" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg w-[550px] p-6">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div><label class="text-xs text-zinc-500">类别</label><input v-model="form.category" list="category-list" class="w-full border rounded px-2 py-1 text-sm" /></div>
          ...
          <div><label class="text-xs text-zinc-500">仓库</label><input v-model="form.warehouse" list="warehouse-list" class="w-full border rounded px-2 py-1 text-sm" /></div>
