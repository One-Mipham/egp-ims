...
import { listPurchases, createPurchase, updatePurchase, deletePurchase, listInventory, listWarehouses } from '../../api'
...
const statusOptions = ['待入库', '已入库', '已取消']
...
async function load() {
  ...
  const { data } = await listPurchases(companyId, params)
  items.value = data
}
