...
const emptyForm = () => ({
  company_id: companyId, order_no: '', customer_name: '', order_date: '',
  product_name: '', quantity: 0, unit: '个', unit_price: 0, total_amount: 0,
  cost_amount: 0, status: '待出库', notes: '',
})
...
async function remove(id: number) {
  if (confirm('确定删除？')) { await deleteInvSale(id); toast.add({ severity: 'success', summary: '已删除', life: 2000 }); await load() }
}

function exportCSV() {
  const header = ['销售单号','客户','产品','数量','单位','单价','金额','成本','毛利','状态','日期']
  const rows = items.value.map((i: any) => [i.order_no, i.customer_name, i.product_name, i.quantity, i.unit, i.unit_price, i.total_amount, i.cost_amount, i.profit, i.status, i.order_date])
  const csv = [header.join(','), ...rows.map((r: any[]) => r.map((c: any) => `"${String(c ?? '').replace(/"/g, '""')}"`).join(','))].join('\n')
  const blob = new Blob(['﻿' + csv], { type: 'text/csv;charset=utf-8;' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url; a.download = '销售单.csv'; a.click()
  ...
  <div class="p-4">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex gap-2">
        ...
        <button @click="openCreate" class="px-4 py-2 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">+ 新增销售单</button>
      </div>
    </div>

    <div class="flex gap-2 mb-3 flex-wrap">
      <input v-model="searchText" @input="page=1; load()" placeholder="搜索单号/客户/产品..." class="border rounded px-2 py-1 text-sm w-56" />
    ...
    <table class="w-full text-sm border-collapse">
      <thead>
        <tr class="bg-zinc-100 text-left">
          <th class="p-2 border">销售单号</th><th class="p-2 border">客户</th><th class="p-2 border">产品</th>
          <th class="p-2 border text-right">数量</th><th class="p-2 border text-right">单价</th><th class="p-2 border text-right">金额</th>
          <th class="p-2 border text-right">毛利</th><th class="p-2 border">状态</th><th class="p-2 border">操作</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="item in items" :key="item.id" class="hover:bg-zinc-50">
          ...
          <td class="p-2 border text-right font-bold" :class="(item.profit || 0) >= 0 ? 'text-green-600' : 'text-red-600'">{{ (item.profit || 0).toLocaleString() }}</td>
    ...
    <div v-if="dialogVisible" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg w-[550px] p-6">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div><label class="text-xs text-zinc-500">销售单号</label><input v-model="form.order_no" class="w-full border rounded px-2 py-1 text-sm" /></div>
          <div><label class="text-xs text-zinc-500">客户</label><input v-model="form.customer_name" list="customer-list" class="w-full border rounded px-2 py-1 text-sm" /></div>
          <div><label class="text-xs text-zinc-500">销售日期</label><input type="date" v-model="form.order_date" class="w-full border rounded px-2 py-1 text-sm" /></div>
          <div>
          ...
          <div><label class="text-xs text-zinc-500">数量</label><input type="number" v-model.number="form.quantity" @input="autoCalc" class="w-full border rounded px-2 py-1 text-sm" /></div>
          <div><label class="text-xs text-zinc-500">单价</label><input type="number" v-model.number="form.unit_price" @input="autoCalc" class="w-full border rounded px-2 py-1 text-sm" /></div>
          <div><label class="text-xs text-zinc-500">金额（自动）</label><input type="number" v-model.number="form.total_amount" readonly class="w-full border rounded px-2 py-1 text-sm bg-zinc-50" /></div>
          <div><label class="text-xs text-zinc-500">成本金额</label><input type="number" v-model.number="form.cost_amount" class="w-full border rounded px-2 py-1 text-sm" /></div>
        </div>
